...
Game.Builder.prototype._generateLevel = function() {
    ...
    // Smoothen it one last thime then update the map
...
Game.Builder.prototype._canFillRegion = function(x, y, z) {
    // Make sure the tile is witin bounds
...
Game.Builder.prototype._fillRegion = function(region, x, y, z) {
    ...
    while (tiles.length > 0) {
        tile = tiles.pop();
        // Get the neighbos of the tile
        neighbors = Game.getNeighborPositions(tile.x, tile.y);
        // Iterate through each neighbor checking if it can use it to fill
        // and if so updating the region and adding to the the processing
        // list
...
// at a given depth level and a region at the level beneath it
Game.Builder.prototype._findRegionOverlaps = function(z, r1, r2) {
    var matches = [];
    // Iterate through all tiles, checking if they respct the region
    // contraints and are floor tiles. Check that they are floor to make
    // sure there isnt two stairs on the same tile
    for (var x = 0; x < this._width; x++) {
        for (var y = 0; y < this._height; y++) {
...
// This tries to connect two regions by calculating where they
// overlap and adding stairs
...
// starting from the top moth depth level
Game.Builder.prototype._connectAllRegions = function() {
    for (var z = 0; z < this._depth - 1; z++) {
        // Iterate through each tile, and if it hasnt already, try
        // to connect the region of that tile on both depth levels.
        // Connected properties are stroed as strings for lookups
        var connected = {};
        var key;
        for (var x = 0; x < this._width; x++) {
            for (var y = 0; y < this._height; y++) {
                ...
                if (this._tiles[z][x][y] == Game.Tile.FloorTile &&
                    ...
                    !connected[key]) {
                        ...
                        // already connected, try to connect now
